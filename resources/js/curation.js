...
export default function curation({ statePath, fileName, fileType, presets = {} }) {
    return {
        ...
        setCropBoxX($event) {
            const selection = this.cropper.getCropperSelection();
            selection.$change(parseInt($event.target.value), selection.y, selection.width, selection.height);
        },
        setCropBoxY($event) {
            const selection = this.cropper.getCropperSelection();
            selection.$change(selection.x, parseInt($event.target.value), selection.width, selection.height);
        },
        setCropBoxWidth($event) {
            const selection = this.cropper.getCropperSelection();
            selection.$change(selection.x, selection.y, parseInt($event.target.value), selection.height);
        },
        setCropBoxHeight($event) {
            const selection = this.cropper.getCropperSelection();
            selection.$change(selection.x, selection.y, selection.width, parseInt($event.target.value));
        },
        flipHorizontally() {
